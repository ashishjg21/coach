          ...
          <div class="flex items-start justify-between gap-3 group pb-4">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <span v-if="item.step" class="text-xs font-bold uppercase tracking-wider" :style="{ color: item.stepColor }">
                  {{ item.step.name }}
                </span>
            ...
            <div class="text-right shrink-0">
              <span class="text-[10px] font-mono font-bold bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded text-gray-600 dark:text-gray-400">
                {{ item.title }}
              </span>
            </div>
          </div>
...
const timelineItems = computed(() => {
  ...
  const totalDuration = props.workout.steps.reduce((acc: number, s: any) => acc + s.durationSeconds, 0)
  ...
  return [...props.workout.messages]
    ...
    .map(msg => {
      ...
      return {
        ...
        zone: stepInfo ? getZone(stepInfo.step.power?.value || (stepInfo.step.power?.range ? (stepInfo.step.power.range.start + stepInfo.step.power.range.end)/2 : 0)) : undefined,
