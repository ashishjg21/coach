...
async function testWhoopIngest() {
  ...
  try {
    ...
    const { fetchWhoopRecovery, fetchWhoopSleep, fetchWhoopWorkouts, normalizeWhoopRecovery, normalizeWhoopWorkout } = await import('../server/utils/whoop')
    ...
    workouts.forEach(w => {
        console.log(`- ${w.date.toISOString()} | ${w.type} | Strain: ${(w.rawJson as any)?.score?.strain} | Avg HR: ${w.averageHr}`)
    })
