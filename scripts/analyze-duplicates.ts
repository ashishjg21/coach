  ...
  for (let i = 0; i < workouts.length; i++) {
    for (let j = i + 1; j < workouts.length; j++) {
        ...
        if (start1 < end2 && end1 > start2) {
            ...
            console.log(`1. [${w1.source}] ${w1.title} (${new Date(start1).toISOString()} - ${new Date(end1).toISOString()})`)
            console.log(`2. [${w2.source}] ${w2.title} (${new Date(start2).toISOString()} - ${new Date(end2).toISOString()})`)
