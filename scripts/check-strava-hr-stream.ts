...
async function checkStravaHRStream() {
  ...
  const { fetchStravaActivityDetails, fetchStravaActivityStreams } = await import('../server/utils/strava')
  ...
  try {
    ...
    const streams = await fetchStravaActivityStreams(
      integration,
      activityId,
      ['heartrate', 'time', 'distance', 'velocity_smooth', 'watts', 'cadence', 'altitude']
    )
