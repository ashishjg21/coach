...
async function main() {
  ...
  if (workoutStream) {
    ...
    if (velocityData && timeData && distanceData) {
        console.log(`Stream lengths: Velocity=${velocityData.length}, Time=${timeData.length}, Distance=${distanceData.length}`)
    ...
    if (workout.rawJson && typeof workout.rawJson === 'object') {
      ...
      if (splits && Array.isArray(splits) && splits.length > 0) {
        ...
        lapSplits.forEach((s: any) => {
            console.log(`Lap ${s.lap}: Dist=${s.distance}m, Time=${s.time}s, Pace=${Math.round(s.paceSeconds)}s/km`)
        })
      } else {
