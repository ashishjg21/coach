...
const compareCommand = new Command('compare')
  .description('Compare local and production database schemas')
  .action(async () => {
    ...
    if (!localConnectionString || !prodConnectionString) {
      console.error(chalk.red('Both DATABASE_URL and DATABASE_URL_PROD must be set in .env'))
      process.exit(1)
    }
    ...
    console.log(chalk.blue('Comparing Full Database Schema: Local vs Production...\n'))
    ...
    try {
      ...
      for (const tableName of Array.from(allTables).sort()) {
        ...
        if (!isLocal) {
          console.log(chalk.red(`❌ Table '${tableName}' is missing in LOCAL database`))
          totalDifferences++
          continue
        }
        if (!isProd) {
          console.log(chalk.red(`❌ Table '${tableName}' is missing in PRODUCTION database`))
          totalDifferences++
          continue
        }
        ...
        if (tableHasDifferences) {
          console.log(chalk.yellow(`⚠️  Differences in table '${tableName}':`))
      ...
      if (totalDifferences > 0) {
        console.log(chalk.red(`❌ Found differences in ${totalDifferences} tables.`))
      } else {
        console.log(
          chalk.green('✅ Full database schema matches perfectly (excluding _prisma_migrations).')
        )
      }
    } catch (error) {
