    ...
    try {
      ...
      for (const user of users) {
        ...
        for (const workout of workouts) {
          ...
          const needsUpdate = Math.abs(currentCtl - ctl) > epsilon || Math.abs(currentAtl - atl) > epsilon;
        ...
        if (options.dryRun) {
          console.log(chalk.yellow(`  [DRY RUN] Would update ${userUpdated} workouts, skip ${userSkipped}.`));
        } else {
